...
controller.loguearse = async (req, res, next) => {
  ...
  const validPass = await bcrypt.compare(clave, data.clave_user);
  if (!validPass) {
    return res.status(401).json({ error: "datos incorrectos" });
  }
  const userForToken = {
...
/*_________________________________________________________________________*/
//REGISTRARSE
controller.registrarse = async (req, res) => {
...
controller.setNote = async (req, res, next) => {
  ...
  const exist = await tableNotes.findOne({
    where: { [Op.and]: [{ userId: `${id}` }, { text_nota: `${cuerpo}` }] },
  });
  if (exist) {
...
/*_________________________________________________________________________*/
//EDITAR NOTA
controller.editNote = async (req, res, next) => {
  ...
  await tableNotes.update(
    ...
    {
      where: {
        [Op.and]: [{ id: `${idNote}` }, { userId: id_user }],
      },
    }
  );
